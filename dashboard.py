...
def load_mappings(file_mod_time_bw=0, file_mod_time_rel=0):
    try:
        ...
        if '物料对应关系' in xls.sheet_names:
            ...
            print("BW数据.xlsx 中未找到物料对应关系 Sheet，尝试从对应关系.xlsx 读取")
            try:
                import os
                if os.path.exists('对应关系.xlsx'):
                    ...
                    if '物料对应关系' in xls2.sheet_names:
                        ...
                        print("未找到【物料对应关系】工作表，尝试遍历所有工作表")
...
def filter_data(df, region, dept, months, flavor, capacity, package, project, product30, product20, region_dept_map, flavor_to_materials, capacity_to_materials, package_to_materials, project_map, product30_map, product20_map, material_col=None):
    ...
    if capacity != '全部' and len(filtered) > 0:
        if capacity_to_materials and capacity in capacity_to_materials and material_col and material_col in filtered.columns:
            materials_to_keep = capacity_to_materials[capacity]
            filtered = filtered[filtered[material_col].astype(str).str.strip().isin(materials_to_keep)]
        elif '容量' in filtered.columns:
            filtered = filtered[filtered['容量'] == capacity]
...
if st.session_state.current_page == '需求分析':
                ...
                if demand_below or budget_below:
                    ...
                    for flavor, diff in flavor_diff.items():
                        ...
                        <div style="padding: 8px 12px; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #e2e8f0;">
                            ...
                            <span style="font-size: 12px; color: {diff_color}; font-weight: 500;">{sign}{diff:.1f}%</span>
                        </div>
                        """
    ...
    with col_right:
        ...
        with card_container:
            ...
            fig_monthly_order.add_trace(go.Bar(
                ...
                orientation='h',
                marker_color='#1e40af',
                text=monthly_order_flavor_df['需求量'].apply(lambda x: f'{x:.1f}'),
            ...
            fig_monthly_order.add_trace(go.Bar(
                ...
                orientation='h',
                marker_color='#60a5fa',
                text=monthly_order_flavor_df['月累排单'].apply(lambda x: f'{x:.1f}'),
                textposition='outside',
                textfont=dict(
                    size=13,
                    weight='bold',
        ...
        with card_container:
            ...
            if '容量' in filtered_df.columns:
                ...
                fig_capacity.add_trace(go.Scatter(
                    ...
                    text=capacity_df['需求量'].apply(lambda x: f'{x:.0f}'),
                    textposition='top center',
                    textfont=dict(color='#1e40af', size=14, weight='bold'),
                    customdata=capacity_df['容量'],
                    hovertemplate='<b>容量:</b> %{customdata}<br><b>需求量:</b> %{y:.0f}<extra></extra>'
                ))
                fig_capacity.add_trace(go.Scatter(
                    x=capacity_df['容量'],
                    y=capacity_df['月累排单'],
                    ...
                    text=capacity_df['月累排单'].apply(lambda x: f'{x:.0f}'),
                    textposition='bottom center',
                    textfont=dict(color='#2563eb', size=14, weight='bold'),
                    customdata=capacity_df['容量'],
                    hovertemplate='<b>容量:</b> %{customdata}<br><b>月累排单:</b> %{y:.0f}<extra></extra>'
                ))
    ...
    with col_a:
        ...
        for _, row in capacity_df.iterrows():
            ...
            gap_rate_display = f"{gap_rate:.1f}%" if (pd.notna(gap_rate) and abs(gap_rate) != float('inf')) else '--'
            ...
            budget_style = "color: #ef4444; font-weight: bold;" if (budget_rate < 100 and pd.notna(budget_rate)) else "color: #22c55e; font-weight: bold;" if (budget_rate >= 100 and pd.notna(budget_rate)) else ""
    ...
    filtered_2026 = filter_data(df_2026_actual, history_region, history_dept, selected_months, history_flavor, history_capacity, history_package, history_project, history_product30, history_product20, REGION_DEPT_MAP, flavor_to_materials, capacity_to_materials, package_to_materials, project_map, product30_map, product20_map, material_col)
    ...
    total_budget = int(pd.to_numeric(filtered_budget[budget_col], errors='coerce').sum()) if not filtered_budget.empty and budget_col in filtered_budget.columns else 0
        ...
        with card_container:
            ...
            if '月份' in filtered_2026.columns and sales_col and sales_col in filtered_2026.columns:
                ...
                fig_monthly_2026.add_trace(go.Bar(
                    ...
                    text=monthly_sales_2026[sales_col].apply(lambda x: f'{int(x):,}' if x > 0 else ''),
                    textposition='outside',
